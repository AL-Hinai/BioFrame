#!/usr/bin/env python3
"""
Manual test script for BioFrame workflow execution
Tests the fixed container mounting and workflow execution
"""
...
def test_manual_workflow():
    """Test the workflow manually with the fixed container mounting"""
    ...
    run_id = f"manual-test-{uuid.uuid4().hex[:8]}"
    workflow_name = f"Manual Test Workflow {run_id}"
    tools = ["fastqc", "trimmomatic", "multiqc"]
    
    # Use the same input files as the failed run
    input_files = [
